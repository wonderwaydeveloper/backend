...
class SessionTimeoutService
{
    ...
    {
        return config('security.tokens.access_lifetime_seconds', 3600);
    }

    public function getRefreshTokenLifetime(): int
    {
        return config('security.tokens.refresh_lifetime_seconds', 604800);
    }

    public function getRememberTokenLifetime(): int
    {
        return config('security.tokens.remember_lifetime_seconds', 1209600);
    }

    public function getSessionTimeout(): int
    {
        return config('security.session.timeout_seconds', 7200);
    }

    public function getConcurrentSessionLimit(): int
    {
        return config('security.session.concurrent_limit', 3);
    }

    public function getAutoRefreshThreshold(): int
    {
        return config('security.tokens.auto_refresh_threshold', 300);
    }

    public function getPasswordResetExpiry(): int
    {
        return config('security.password.reset.expire_minutes', 15); // minutes
    }

    public function getEmailVerificationExpiry(): int
    {
        return config('security.email.verification_expire_minutes', 15) / 60; // convert minutes to hours
    }

    public function getDeviceTokenInactivityLimit(): int
    {
        return config('security.device.max_inactivity_days', 30); // days
    }

    public function getVerificationCodeExpiry(): int
    {
        return config('security.email.verification_expire_minutes', 15); // minutes
    }

    public function getTwoFactorCodeExpiry(): int
    {
        return config('security.tokens.auto_refresh_threshold', 300) / 60; // minutes
    }
